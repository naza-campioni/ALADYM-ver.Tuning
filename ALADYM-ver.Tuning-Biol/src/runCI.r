...
print("SONO IN RUN CI ENV", quote=F)
...
if (new_aldForecast@CI_recruitment) {
       ...
       if ( new_aldForecast@CI_recruitment_2_SRR_model  ==   "Beverton and Holt") {
         INP$FRLt_fore <-  1
          ...
          INP$FRLt_fore <-  6
        } else if ( new_aldForecast@CI_recruitment_2_SRR_model  ==   "Beverton and Holt env") {
          INP$FRLt_fore <-  7
        }  else if ( new_aldForecast@CI_recruitment_2_SRR_model  ==   "Ricker env") {
          INP$FRLt_fore <-  8
       }
...
INP$FRLd_fore  <- as.numeric(INP$SRR_CI_a_b_grid[numero_ciclo,4]) 
...
if(BMT_SPECIES[ALADYM_spe]=="M.mer1718" ){
  ...
  ENV<<-read.table("C:\\Training\\INPUT\\ENVIRONMENTAL_GRIDS\\HKE1718_50_500_RCP45_bia_adj_month_2.csv",sep=";",header=T)  
  
} else if (BMT_SPECIES[ALADYM_spe]=="M.mer19" ){
  ENV<<-read.table("C:\\Training\\INPUT\\ENVIRONMENTAL_GRIDS\\HKE19_50_500_RCP45_bia_adj_month_2.csv",sep=";",header=T)  
...
}                                                                                             # ARSARA181920_200_500_RCP45_bia_adj_month_2
