# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/saveObservedvsSimulatedPlot_landing.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: saveObservedvsSimulatedPlot_landing) {')
...
dir_to_create <- file.path(dirname(SIMULATEDVSOBSERVED_LANDING_graph), CAL$combo_tag)
...
path_landing_table <- tag_output_path(SIMULATEDVSOBSERVED_LANDING_table, tag)
path_plot          <- tag_output_path(SIMULATEDVSOBSERVED_LANDING_graph, tag)
path_chi_table     <- tag_output_path(CHISQUARED_LANDING_table, tag)  

write.table(landing_table, path_landing_table, col.names=FALSE, row.names=FALSE, sep=";")
