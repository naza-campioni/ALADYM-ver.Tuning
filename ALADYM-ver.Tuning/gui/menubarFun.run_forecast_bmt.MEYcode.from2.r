# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/gui/menubarFun.run_forecast_bmt.MEYcode.from2.r'), '\n')
    ...
    NPV15_calculated <- Fleetyear[[simperiod]]@fleetsegments[[fleet_i]]@EC.NPV.discounted 
     NPV15_calculated_perc <- Fleetyear[[simperiod]]@fleetsegments[[fleet_i]]@EC.NPV.discounted.CI.perc
       for (ye_f in (simperiod+1):(simperiod+numb_yearsNPV)) {
          ...
          bmtindicators_discount_rate <- convert_numeric0_to_na(as.numeric(as.character(eimat[8,fleet_i])))  
     for (ye_f in 1:foreperiod) {
