# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/gui/biological/biologicalFun/setBiologicalParams.r'), '\n')
...
convert_numeric0_to_na <- function(x) {
  if (is.null(x)) return(x)
  # handle numeric vectors, data frames, matrices, etc.
...
# if (printOK) if (printOK)   print('Calling function: setBiologicalParamsBIOmatr) {')
...
# tr_value <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(BIOmatr$Distribution[BIOmatr$Parameters == "Tr"]))))

gtkEntrySetText(entryOFFSPRING_tr, tr_value)

# BIOmatr[,2] <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(BIOmatr[,2]))))
...
stockrecruitment.SRvector <<- try(read.csv(up_file_Recr, sep=";"),silent=T )
...
mortality.Mvector.males <<- try(read.csv(up_file_M_males, sep=";", na.strings = "") ,silent=T )
...
mortality.Mvector.females <<- try(read.csv(up_file_M_females, sep=";", na.strings = ""),silent=T  )
...
mortality.Zvector <<- try(read.csv(up_file_Z, sep=";", na.strings = ""),silent=T )
...
print(paste("--------------------- Error in reading TOTAL MORTALITY file [path: \"", up_file_Z, "\"]", sep =""), quote=F)
    ...
    loaded_Fmortality <- try(read.csv(up_file_F, sep=";", na.strings = ""),silent=T  )  
    print("--------------------- Loading fishing mortality configuration", quote=F )

    loaded_Fmortality_overall <- try(read.csv(up_file_F_overall, sep=";", na.strings = "") ,silent=T )  
...
loaded_selectivity <- try(read.csv( up_file_sel, sep=";", na.strings = "") ,silent=T  )
print("--------------------- Loading selectivity configuration", quote=F)
  ...
  if (!is.na(up_file_sel_age)) {
    print(up_file_sel_age)
if (nchar(up_file_sel_age ) > 1) {

loaded_selectivity_age <- try(read.csv( up_file_sel_age, sep=";", na.strings = "") ,silent=T )
...
loaded_selectivity_length <- try(read.csv( up_file_sel_length, sep=";", na.strings = "") ,silent=T  )
print("--------------------- Loading selectivity by length configuration", quote=F)
    ...
    if (F_TYPE == "O" ) {
        ...
        loaded_CatchatAGE <- try(read.csv( up_file_catch_for_splitting, sep=";", na.strings = ""),silent=T   )
    
         FF_overall_mat <- try(read.csv( up_file_F_overall, sep=";", na.strings = ""),silent=T   )
         ...
         FM_overall_mat <- try(read.csv( up_file_F_overall, sep=";", na.strings = "") ,silent=T  )  
...
if (IN_BEMTOOL) {
  ...
  Production_TYPE <- "P"
    ...
    if (!IN_BEMTOOL) {
if (Effort_TYPE == "E") {
loaded_effortvariables <- try(read.csv( up_file_eff, sep=";", na.strings = "") ,silent=T  )
print("--------------------- Loading effort data configuration", quote=F)
} else {
loaded_fishcoeff <- try(read.csv( up_file_fc, sep=";", na.strings = ""),silent=T ) 
...
if (Production_TYPE == "P") {
    if (!IN_BEMTOOL) {
loaded_production <- try(read.csv(up_file_P, sep=";", na.strings = "") ,silent=T ) 
print("--------------------- Loading production configuration", quote=F )
}
loaded_monthly_discard <- try(read.csv(up_file_D, sep=";", na.strings = ""),silent=T  ) 
print("--------------------- Loading discard configuration", quote=F )
} else {
    if (!IN_BEMTOOL) {
loaded_PP <- try(read.csv( up_file_PP, sep=";", na.strings = ""),silent=T  ) 
...
loaded_discard <- try(read.csv(up_file_dis, sep=";", na.strings = "") ,silent=T ) 
print("--------------------- Loading discard reverse ogive configuration", quote=F )
} 

if (any(Discard_TYPE_notNA == "V")) {
loaded_discard_extvector <- try(read.csv(up_file_dis_external, sep=";", na.strings = "") ,silent=T ) 
print("--------------------- Loading discard external vector configuration", quote=F )
}

}

loaded_landing_obligation <- try(data.frame(read.csv(up_file_landing_obl, sep=";", na.strings = "")) ,silent=T ) 
...
if (any(Escape_survivability_DOS_TYPE_notNA[!is.na(Escape_survivability_DOS_TYPE_notNA)] == "V")) {
  loaded_escape_surv_external_vector <- try(data.frame(read.csv(up_file_escape_surv_ext_vect, sep=";", na.strings = "")) ,silent=T ) 
...
effF$a <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(EffortF_RELATIONSHIP_a[1,fs] ))))
effF$b <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(EffortF_RELATIONSHIP_b[1,fs] ))))
...
if (Mortality_TYPE == "F") {
   ...
   if (F_TYPE == "O") {
    ...
    F_overall_matrix <<- try(read.csv( up_file_F_overall, sep=";", na.strings = "")  ,silent=T )  
...
FleetList_forecast[[fs]]@discard.calculation <<- "NA" 
  ...
  # ?????????????????????????????????????????????????????????????????????????????set discard survivability component
...
FleetList_forecast[[fs]]@discard.survivability.calculation <<- "N" 
  ...
  # ?????????????????????????????????????????????????????????????????????????????set escape survivability component
...
disc_data$Discard <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(disc_data$Discard))))
disc_data$Discard[is.na(disc_data$Discard)] <- 0
prod_data$Production <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(prod_data$Production))))  +  as.numeric(as.character(disc_data$Discard))
}
...
fishcoeff_fs <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(fact_mat[,fs] ))))
...
if (Production_TYPE == "P" ) {
if (class(loaded_production ) != "try-error") {
  pp_fs <- convert_numeric0_to_na(as.numeric(as.character(convert_numeric0_to_na(p_production_mat[,fs]))))
