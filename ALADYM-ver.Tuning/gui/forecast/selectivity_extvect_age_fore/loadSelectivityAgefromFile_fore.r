# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/gui/forecast/selectivity_extvect_age_fore/loadSelectivityAgefromFile_fore.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: loadSelectivityAgefromFile_forew) {')
dialog <- gtkFileChooserDialog("Choose a CSV file", main_window, "open", "gtk-cancel", GtkResponseType["cancel"], "gtk-open", GtkResponseType["accept"])
if (dialog$run() == GtkResponseType["accept"]) {
...
#n_ages_M  <- convert_numeric0_to_na(as.numeric(as.character(gtkEntryGetText(entryVBF_M_lifespan))))  
#n_ages_F  <- convert_numeric0_to_na(as.numeric(as.character(gtkEntryGetText(entryVBF_F_lifespan))))  
