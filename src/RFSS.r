# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/RFSS.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: RFSSS_unit, para_SS_Population, para_RLt, para_RLa, para_RLb, para_RLc, para_R) {')
...
para_R = NA #INP$Recruits[loca_irun + 1]
  ...
  if(para_RLt == 1) {
     loca_temp <- para_SS_Population / (para_RLa + para_RLb * para_SS_Population)  * 1000         # il numero di reclute  in migliaia, devo trasformarlo il numeri assoluti
              
  } else if (para_RLt == 2) {
    loca_temp <- para_RLa * para_SS_Population * exp(-para_RLb * para_SS_Population) * 1000                  # il numero di reclute  in migliaia       
  
  } else if (para_RLt == 3) {
    loca_temp <- para_RLa * para_SS_Population / (1 + (para_SS_Population / para_RLc)^para_RLb)     * 1000       # il numero di reclute  in migliaia
  
  } else if (para_RLt == 4) {
    loca_temp <- para_R  * 1000       # il numero di reclute  in migliaia
  
  } else if(para_RLt == 5) {
    loca_temp <- para_RLa * min(para_SS_Population, para_RLb)  * 1000       # il numero di reclute  in migliaia
