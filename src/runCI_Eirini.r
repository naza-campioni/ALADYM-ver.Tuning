# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/runCI.r'), '\n')
...
for (nco in 2:(ncol(recruitment_percs)-1)) {
    if (allYears[yea] %in% years) {   
		...
		recruitment_percs[recruitment_percs$Year == allYears[yea] & recruitment_percs$percentile == percentiles_numb[perc] ,nco] <- convert_numeric0_to_na(as.numeric(as.character(quantile(as.numeric(as.character(Recruitment_ALLruns_thisye[,nco])))))) , probs =  c(0.05,0.25,0.5,0.75,0.95), na.rm=T )[perc]   ))
    }
}
