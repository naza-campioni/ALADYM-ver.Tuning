# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/runCI.int.r'), '\n')
...
# errore pu venire dall'esterno (opzione 1) oppure pu essere calcolato da aladym   (opzione 2 )
...
for (nco in 2:(ncol(INP$recruitment_percs)-1)) {
    ...
    percs_to_add[1,nco] <- convert_numeric0_to_na(as.numeric(as.character(quantile(as.numeric(as.character(Recruitment_ALLruns_thisye[,nco])))))) , probs =  c(0.05,0.25,0.5,0.75,0.95), na.rm=T )[perc]   ))
...
for (nco_name in mortalities_CI_selection) {
    ...
    # INP$mortalities_percs[INP$mortalities_percs$Year == allYears[yea] & INP$mortalities_percs$percentile == percentiles_numb[perc] ,nco] <- convert_numeric0_to_na(as.numeric(as.character(quantile(as.numeric(as.character(mortalities_ALLruns_thisye[,nco])))))) , probs =  c(0.05,0.25,0.5,0.75,0.95), na.rm=T )[perc]   ))  
    }
}
...
for (nco_name in Population_CI_selection) {
    ...
    # INP$population_percs[INP$population_percs$Year == allYears[yea] & INP$population_percs$percentile == percentiles_numb[perc] ,nco]<- convert_numeric0_to_na(as.numeric(as.character(quantile(as.numeric(as.character(population_ALLruns_thisye[,nco])))))) , probs =  c(0.05,0.25,0.5,0.75,0.95), na.rm=T )[perc]   )) 
		}
...
#  #	INP$discard_percs[INP$discard_percs$Year == allYears[yea] & INP$discard_percs$percentile == percentiles_numb[perc] ,nco]  <- convert_numeric0_to_na(as.numeric(as.character(quantile(as.numeric(as.character(discard_ALLruns_thisye[,nco])))))), probs =  c(0.05,0.25,0.5,0.75,0.95), na.rm=T )[perc]   ))
#		}
#}
#
# if ( ( (allYears[yea] %in% years) & (current_year == 1) ) | years_forecast[current_year] ==  allYears[yea] ) {
...
for (nco_name in Indicators_CI_selection) {
    ...
    # INP$indicators_percs[INP$indicators_percs$Year == allYears[yea] & INP$indicators_percs$percentile == percentiles_numb[perc] ,nco]  <- convert_numeric0_to_na(as.numeric(as.character(quantile(as.numeric(as.character(indicators_ALLruns_thisye[,nco])))))), probs =  c(0.05,0.25,0.5,0.75,0.95), na.rm=T )[perc]   ))
