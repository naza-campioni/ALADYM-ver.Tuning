# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/gui/utilities/interconn.bmt/update_BMT_fleetsegments_fore.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: update_BMT_fleetsegments_forenum_ORD_fleetsegm, num_fleet) {')
...
yea <- yea_f + simperiod
vess <- convert_numeric0_to_na(as.numeric(as.character(Fleetyear[[yea]]@fleetsegments[[num_fleet]]@VESSELS )))
# print(vess)
...
for (yea_f in 1:foreperiod) {
yea <- yea_f + simperiod
days <- convert_numeric0_to_na(as.numeric(as.character(Fleetyear[[yea]]@fleetsegments[[num_fleet]]@DAYS.average )))
...
for (yea_f in 1:foreperiod) {
yea <- yea_f + simperiod
gts <- convert_numeric0_to_na(as.numeric(as.character(Fleetyear[[yea]]@fleetsegments[[num_fleet]]@GT.average )))
...
#}
#discMatr[,3] <- convert_numeric0_to_na(as.numeric(as.character(discMatr[,3]))) 
#discMatr[,4] <- convert_numeric0_to_na(as.numeric(as.character(discMatr[,4]))) 


n_ages_males <- convert_numeric0_to_na(as.numeric(as.character(Populations[[ALADYM_spe]]@lifespan[1,1])))
...
n_ages_females <- convert_numeric0_to_na(as.numeric(as.character(Populations[[ALADYM_spe]]@lifespan[2,1])))
