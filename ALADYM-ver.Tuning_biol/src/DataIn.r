# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/DataIn.r'), '\n')
...
RND$R_fun                  <- which(DISTRIBUTION == new_aldSimulation@R.100runs$distribution[1])# reclutamento IN MIGLIAIA funzione di distribuzione di probabilit e suoi parametri                                                                          
...
for(loca_i in (11:1)) {
  INP$Fertility_Rate[loca_i + 1] <- INP$Fertility_Rate[loca_i]         # si tiene conto del ritardo di un mese e si riscrive il vettore delle fertilit, traslando il tutto di un mese
}
...
INP$Type = vector(mode="character", length=length(FLEETSEGMENTS_names)) # A (vettore per et, AGE) o L (vettore per lunghezza/LENGTH)
...
INP$Esc_Vector_sim_temp[1:(n_ages_M),(1+fs)] <- convert_numeric0_to_na(as.numeric(as.character(col_to_add_M_Sur_Vector)))
INP$Esc_Vector_sim_temp[(n_ages_M+1):(n_ages_M+n_ages_F),(1+fs)] <- convert_numeric0_to_na(as.numeric(as.character(col_to_add_F_Sur_Vector)))
...
# Altri parametri di selettivit: inizializzazione -------------------------------------------------------
...
for (gear in 1:nb_gears){
      # fishing_effort_temp = gear_data[gear_data$Gear == FLEETSEGMENTS_names[gear],]
       ...
       row_prod <- convert_numeric0_to_na(as.numeric(as.character(row_prod[row_prod!=""])))
       ...
       row_effort <- convert_numeric0_to_na(as.numeric(as.character(row_effort[row_effort!=""])))
...
#      INP$p_Production[loca_i,gear]<- fishing_effort_temp$p_production[loca_i]
        ...
        INP$OPT_SG_TYPE[loca_i,gear]   <- FleetList_simulation[[gear]]@selectivity.vector[loca_i,8]   # tipo di selettivit
...
# spezzare i mesi simulation + forecast in tempi diversi
      ...
      # il P_production lo calcola solo se c' entrata con Z                                                           
for (gear in 1:nb_gears){
...
# } 
 ...
 if (FleetList_simulation[[gear]]@discard.datatype == "Reverse ogive") {
   ...
   INP$param6[loca_i,gear]  <- convert_numeric0_to_na(as.numeric(as.character(FleetList_simulation[[gear]]@discard.vector[loca_i, 3]  )))
