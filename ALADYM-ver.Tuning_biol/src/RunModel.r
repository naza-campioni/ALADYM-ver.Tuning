# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/RunModel.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: RunModelStart,End) {')


  
  #!!!  

  # load_firstStepF=firstStepF
  # load_firstStepF_entrataF=firstStepF_entrataF
  # load_firstStepM_entrataF=firstStepM_entrataF
  # load_firstStepM=firstStepM
...
if (as.numeric(INP$OPT_F_TYPE)==2) {
 try( load_firstStepM_entrataF() ,silent=T)
  } else { 
  try(load_firstStepM(BAS$MZ_estimated[1], INP$param1[1,],INP$param2[1,],INP$param3[1,],INP$param4[1,],INP$param5[1,]),silent=T)
  }
  ...
  # RANDOM RUNS Females
 if (as.numeric(INP$OPT_F_TYPE)==2) {
try(  load_firstStepF_entrataF() ,silent=T)
  }else {
try( load_firstStepF(BAS$FZ_estimated[1], INP$param1[1,],INP$param2[1,],INP$param3[1,],INP$param4[1,],INP$param5[1,]),silent=T)
  }
...
INP$Esc_Vector_sim_temp[INP$Esc_Vector_sim_temp$Sex == "M",i+1] <- convert_numeric0_to_na(as.numeric(as.character(FleetList_simulation[[i]]@escape.survivability.DOS.ext_vect.M[1,]))) 
INP$Esc_Vector_sim_temp[INP$Esc_Vector_sim_temp$Sex == "F",i+1] <- convert_numeric0_to_na(as.numeric(as.character(FleetList_simulation[[i]]@escape.survivability.DOS.ext_vect.F[1,])))
}
} 
...
INP$Esc_Surv_rate_vectF_sim = INP$Esc_Surv_rate_vectF_sim[-1,]
  ...
  #-----------------------------
if (as.numeric(INP$OPT_F_TYPE)==2) {
try(  load_PRELIFE_EXPLOITED_entrataF(loca_Fertility,loca_min_BAS.MM,loca_min_BAS.FM),silent=T)
  } else {
  try(load_PRELIFE_EXPLOITED(loca_Fertility,loca_min_BAS.MM,loca_min_BAS.FM),silent=T)
  }
  try(load_PRELIFE_UNEXPLOITED(loca_Fertility,loca_min_BAS.MM,loca_min_BAS.FM),silent=T)
...
# inizializzazione delle 4 popolazioni e delle rispettive reclute e riproduttori
 ...
 try(  load_SIMULATION_UNEXPLOITED(loca_Fertility, Start, End) ,silent=T)    # GLO$L_number 
   
     write.table(INP$Recruits, file="reclute mensili_prima di SIMULATION_EXPLOITED.csv", sep=";")    
   if (as.numeric(INP$OPT_F_TYPE)==2) {
 try(  load_SIMULATION_EXPLOITED_entrataF(loca_Fertility,loca_min_BAS.MM,loca_min_BAS.FM, Start,End) ,silent=T)    
   } else {
 try(  load_SIMULATION_EXPLOITED(loca_Fertility,loca_min_BAS.MM,loca_min_BAS.FM, Start,End) ,silent=T)    
   }
  #----------------------------- 
