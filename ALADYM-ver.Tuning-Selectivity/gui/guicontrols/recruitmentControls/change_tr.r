# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/gui/guicontrols/recruitmentControls/change_tr.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: change_trw) {')

go_on_tr <- TRUE
 Tr <<- convert_numeric0_to_na(as.numeric(as.character(gtkEntryGetText(entryOFFSPRING_tr)))) 
...
#   if (nr==1 ) {
#       mortality_temp_M <- convert_numeric0_to_na(as.numeric(as.character(Populations[[ALADYM_spe]]@M.vect$M[nr,] )))
#   } else {
...
#   if (nr==1) {
#       mortality_temp_F <- convert_numeric0_to_na(as.numeric(as.character(Populations[[ALADYM_spe]]@M.vect$F[nr,] )))
#   } else {
