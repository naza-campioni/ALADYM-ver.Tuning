# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/gui/forecast/discard_extvectorFun_fore/loadDiscards_extvectorfromFile_fore.r'), '\n')
...
loadFfromFile<-function(w) {
# if (printOK) if (printOK)   print('Calling function: loadFfromFilew) {')
dialog <- gtkFileChooserDialog("Choose a CSV file", main_window, "open", "gtk-cancel", GtkResponseType["cancel"], "gtk-open", GtkResponseType["accept"])
if (dialog$run() == GtkResponseType["accept"]) {
loaded_F <- read.csv(dialog$getFilename(), sep=";", na.strings = "")  
vai <- T
