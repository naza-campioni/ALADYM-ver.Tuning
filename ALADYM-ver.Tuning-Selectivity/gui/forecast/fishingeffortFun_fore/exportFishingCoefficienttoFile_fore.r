# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/gui/forecast/fishingeffortFun_fore/exportFishingCoefficienttoFile_fore.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: exportFishingCoefficienttoFile_forew) {')
...
colnames(proddata_table) <- heading
 ...
 years_rep <- rep(years_forecast, 12)
   ...
   proddata_table$FISHING_COEFFICIENT <- convert_numeric0_to_na(as.numeric(as.character(fact_mat_fore[c(length(years)*12+2):nrow(fact_mat_fore), fs])))
...
colnames(proddata_table) <- heading
 ...
 years_rep <- rep(years_forecast, 12)
   ...
   for (yef in 1:length(years_forecast) ) {
     proddata_table$FISHING_COEFFICIENT[( (yef-1)*12+1) : (yef*12)] <- convert_numeric0_to_na(as.numeric(as.character(FleetList_forecast[[fs]]@fishingeffort.vector[yef,2:13])))
   }
