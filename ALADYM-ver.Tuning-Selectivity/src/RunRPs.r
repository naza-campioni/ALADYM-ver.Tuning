# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/RunRPs.r'), '\n')
...
try(load_SIMULATION_UNEXPLOITED(loca_Fertility,1,(loca_Nsimulation2-1)),silent=T) #questo deve stare prima dell'exploited! 
if (as.numeric(INP$OPT_F_TYPE)==1) {  
try(load_SIMULATION_EXPLOITED(loca_Fertility,loca_min_BAS.MM,loca_min_BAS.FM, 1, (loca_Nsimulation2-1)),silent=T) 
} else {
try(load_SIMULATION_EXPLOITED_entrataF(loca_Fertility,loca_min_BAS.MM,loca_min_BAS.FM, 1, (loca_Nsimulation2-1)),silent=T) 
}

# esportazione dei risultati
#source("src/export.r")
 tryLog(load_Annual_Z_Sinclair((loca_Nsimulation2-1)))
        tryLog(load_Annual_F_weighted((loca_Nsimulation2-1)))
               tryLog(load_Annual_Z(loca_Nsimulation2-1))
                      tryLog(load_Annual_F(loca_Nsimulation2-1))
