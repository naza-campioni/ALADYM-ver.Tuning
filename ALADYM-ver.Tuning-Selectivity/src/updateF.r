# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/updateF.r'), '\n')
...
# if (printOK) if (printOK)   print('Calling function: updateFFm,loca_Nsimulation2){')
gears = ncol(Fm)-2
...
Fbygear = data.frame(matrix(nrow = ((loca_Nsimulation2-1)*ages/12),ncol=gears))
  for (i in 1:gears){
    ...
    matrixF[y,] = Fm_temp[((y-1)*ages+1):(y*ages),]     
         # matrice per 1 attrezzo per anni (righe) - et (colonne)
    }
