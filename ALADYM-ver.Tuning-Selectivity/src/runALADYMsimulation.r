# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/runALADYMsimulation.r'), '\n')
...
if (!SKIP_spe) {
  ...
  if (gtkToggleButtonGetActive(radio_data)) {
    ...
    if (as.numeric(INP$OPT_F_TYPE)==1) {      # il P_production lo calcola solo se c'? entrata con Z                                                           
  ...
  for (nr in 1:nrow(sim_land)) {
    sim_catch[nr,] <- convert_numeric0_to_na(as.numeric(as.character(sim_land[nr,])))  + as.numeric(as.character(sim_disc[nr,]  ))
  }
  ...
  for (nr in 1:nrow(obs_land)) {
    obs_catch[nr,] <- convert_numeric0_to_na(as.numeric(as.character(obs_land[nr,])))  + as.numeric(as.character(obs_disc[nr,]  ))
  }
  ...
  CAL$summary_table <- summary_table
  
  dir_to_create <- file.path(dirname(SIMULATEDVSOBSERVED_SUMMARY_table), CAL$combo_tag)
  ...
  path_summary_table <- tag_output_path(SIMULATEDVSOBSERVED_SUMMARY_table, tag)
  write.table(summary_table, path_summary_table, col.names=FALSE, row.names=FALSE, sep=";")
