# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/Refpoints_interpolazione_poli5grado_no_SRR.r'), '\n')
...
biomass1 <<- YpR_data$Total_biomass_exploited_pop/recruitment
...
mod.nls_b <- try(nls(biomass1~exp(a+x*b),start=list(a=1,b=1)),silent=T)
...
B_max_obs  <-biomass1[which(y==max(y))] 
B_01  <- exp(coeff_B[1]+coeff_B[2]*F01 ) 
B_01_obs <-biomass1[which(z_==max(z_))]      
B_02  <-exp(coeff_B[1]+coeff_B[2]*F02 )   
B_02_obs<-biomass1[which(t_==max(t_))]
} else {
B_max_obs  <-biomass1[which(y==max(y))]
B_max <- NA
B_01  <- NA
