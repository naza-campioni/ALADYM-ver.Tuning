cat('Running script:', 'DataIn_forecast.r', '\n')
# if (printOK) if (printOK) cat('Sourcing file: ', basename('C:/Users/emant/Downloads/stecf_tech_measures-main3/stecf_tech_measures-main/BEMTOOL2.5/code/src/biol/bmtALADYM/ALADYM-ver12.3-2017_0501/src/DataIn_forecast.r'), '\n')
...
# *********************************************************************************  MATRICI DELLA SELETTIVIT? PER ET? E LUNGHEZZA (INIZIALIZZAZIONE)
...
# *********************************************************************************  MATRICI DELLA SELETTIVIT? PER ET? E LUNGHEZZA (ASSEGNAZIONE)
...
INP$COEF2[gear] <- convert_numeric0_to_na(as.numeric(as.character(FleetList_forecast[[gear]]@EffortF.relationship$b)))
...
for (loca_i in 1:(12*foreperiod)) {  
 ...
 if (FleetList_forecast[[gear]]@discard.datatype == "Reverse ogive") {
   ...
   INP$param6[loca_i+forecast,gear]  <- convert_numeric0_to_na(as.numeric(as.character(FleetList_forecast[[gear]]@discard.vector[loca_i, 3])))
   INP$param7[loca_i+forecast,gear]  <- convert_numeric0_to_na(as.numeric(as.character(FleetList_forecast[[gear]]@discard.vector[loca_i, 4] )))
   } else {
...
INP$Esc_Vector_fore_temp[INP$Esc_Vector_fore_temp$Sex == "M",i+1] <- convert_numeric0_to_na(as.numeric(as.character(FleetList_forecast[[i]]@escape.survivability.DOS.ext_vect.M[1,]))) 
INP$Esc_Vector_fore_temp[INP$Esc_Vector_fore_temp$Sex == "F",i+1] <- convert_numeric0_to_na(as.numeric(as.character(FleetList_forecast[[i]]@escape.survivability.DOS.ext_vect.F[1,])))
}
} 
...
# ------------------------------------------------------- SENZA incertezza sul reclutamento  
  if (! new_aldForecast@CI_recruitment) {
    ...
    if (new_aldForecast@CI_recruitment_constant_or_SRR == 1) {
          ...
          INP$Recruits_original[(forecast+1):((simperiod+foreperiod)*12 +1)] <- convert_numeric0_to_na(as.numeric(as.character(new_aldForecast@CI_recruitment_1_value))) 
